...
export const aiEditorialGuides: Record<string, AiEditorialGuide> = {
  ...
  midjourney: {
    verdict: "Midjourney适合把图片创作当作持续工作的人，而不是‘偶尔玩一次’的新手：它的优势在视觉风格、参考图、编辑、变化和素材管理，代价是需要订阅并认真处理公开范围与授权。",
    plainDefinition: "Midjourney是一款以图片和视频创作为中心的生成式AI。你在网页Create页面输入画面描述，系统生成多张候选；之后可以做变化、放大、扩图、局部重绘、风格参考、个性化和图片转视频。它不是普通聊天助手，也不是传统精确排版软件。",
    chooseIf: ["每周都会制作概念图、营销视觉、故事画面或风格探索", "需要用参考图、情绪板和个性化保持视觉方向", "愿意学习提示词、版本管理、素材授权和套餐用量"],
    skipIf: ["只偶尔生成一两张图片，订阅成本可能不划算", "素材必须默认完全私密，但尚未选择支持Stealth的套餐"],
    workflows: [
      { title: "从文字需求做出第一轮视觉方向", situation: "你有一个活动、产品或故事概念，但还没有明确画面。", steps: ["把主体、场景、构图、光线和用途写成一句提示词", "生成后只比较构图与方向，不急着处理所有细节", "选中最接近的一张，再用Vary或Editor逐步修改"], result: "得到一组可以讨论和继续迭代的视觉候选，并保留每次提示词与结果。", caution: "AI图像可能出现文字、手部、标志和事实细节错误，不能不检查就直接商用。" },
      { title: "用参考图保持品牌或角色方向", situation: "多张图片需要在色彩、质感、角色或物体上保持相近。", steps: ["确认参考图有合法使用权", "选择Image Prompt、Style Reference或Omni Reference的正确用途", "先小批量生成并比较一致性，再扩大数量"], result: "建立更稳定的风格和角色方向，减少每次从零描述。", caution: "参考不等于精确复制；人物肖像、品牌资产和受版权保护素材仍要获得授权。" },
      { title: "从生成图进入编辑、整理和交付", situation: "已经选到合适方向，需要修改局部、扩图并交给团队。", steps: ["在Editor选择需要重绘或扩展的区域", "在Organize中按项目、日期或文件夹保存版本", "下载前检查尺寸、细节、公开范围和最终授权"], result: "形成有版本、可追溯、能继续修改的视觉素材包。", caution: "不要只保留最终图片；同时保存提示词、参考图来源和修改记录，方便复现与证明授权。" },
    ],
    features: [
      { name: "Create / Imagine", plain: "输入文字或图片参考并生成一组视觉结果。", example: "为产品发布做三种不同情绪的横版主视觉。" },
      { name: "Style与Omni参考", plain: "分别控制视觉风格，或让人物和物体在多张图中更接近。", example: "保持同一角色特征，同时更换场景、服装和镜头。" },
      { name: "Editor", plain: "局部重绘、缩放、扩展画面、图层和重新纹理。", example: "保留人物与光线，只替换背景中的桌面物品。" },
      { name: "Organize与Moodboards", plain: "按项目整理作品，并用情绪板建立长期视觉方向。", example: "把品牌色、材质、构图和参考图整理成统一风格。" },
    ],
    starterTask: { title: "第一次测试：用一个需求完成三轮迭代", prompt: "为【真实用途】生成一张【画面比例】视觉。主体是【主体】，场景是【场景】，光线与质感为【方向】；不要出现文字、商标和无法授权的人物。", check: ["是否能说清哪一轮修改解决了什么问题", "是否检查手部、文字、标志和事实细节", "是否保存提示词、参考图来源和最终授权说明"] },
    decision: "持续做图片、风格和视觉迭代时再选Midjourney；只偶尔做图或需要综合问答、文件与办公时，先用ChatGPT等通用AI的图片功能更省事。",
  },
};
