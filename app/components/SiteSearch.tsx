...
export default function SiteSearch({ entries }: { entries: SearchEntry[] }) {
  ...
  return (
    <div className="site-search">
      <form role="search" onSubmit={submitSearch}><label htmlFor="site-search-input">输入产品、系统、问题或服务名称</label>
      <div className="site-search-bar"><input id="site-search-input" value={query} onChange={(event) => setQuery(event.target.value.slice(0, 80))} placeholder="例如：ChatGPT、月付、iPhone、Clash、账号交付" autoComplete="off" enterKeyHint="search" aria-controls="site-search-results" /><button type="button" onClick={() => setQuery("")} disabled={!query}>清空</button></div></form>
      ...
      <div className="search-results" id="site-search-results">{results.map((entry) => <Link href={entry.href} key={`${entry.href}-${entry.title}`}><span>{entry.category}</span><strong>{entry.title}</strong><p>{entry.description}</p><b>打开内容 →</b></Link>)}</div>
